...
import com.customer.pereference.exception.DataNotFoundException;
import com.customer.pereference.model.BrandModels;
...
import com.customer.pereference.repository.BrandModelCategoryRepository;
import com.customer.pereference.repository.BrandModelsRepository;
import com.customer.pereference.repository.BrandsRepository;
import com.customer.pereference.repository.CustomerRepository;
...
//        if (brandId == 0) {
//            throw new DataNotFoundException(HttpStatus.NOT_FOUND.value(), ErrorResponse.INVALID_BRAND_ID.message, ErrorResponse.INVALID_BRAND_ID.message);
//        }
...
////            customerMap = supplierList.stream().collect(Collectors.toMap(Supplier::getId, Function.identity()));
...
////        }
////
////        Map<Long, Supplier> finalSupplierHashMap = customerMap;
...
//        return  return brands.stream().map(brand -> brandMapper.convertBrandsToBrandsBo(brand)).collect(Collectors.toList());
//    }
//
...
//
//    @Transactional
//    public List<BrandBO> getBrandBySupplier(Long customerId) {
//        List<Brands> brandList = null;
//        List<BrandBO> brands = new ArrayList<>();
//        try {
//            brandList = brandsRepository.findBrandByCustomerId(customerId);
...
//                Customer customer = customerRepository.findById(Long.valueOf(supplierId)).get();
//                BrandModels brandModels = brandModelsRepository.findByBrands(brands);
//                List<Products> productList = productRepository.findByCustomerId(customer);
...
//        try {
//            Optional<Supplier> suppOpt = customerRepository.findById(supplierId);
...
//            return new ResponseEntity<>(successResponse, HttpStatus.OK);
//        }
//        Supplier supplier = customerRepository.findByEmail(supplierEmail);
//        if (supplier == null) {
